...
# Danh sách câu chửi + sex
insults = [
    "Đụ má mày @{} ngu vl",
    "Cặc lồn mẹ mày {} con chó",
    "Tao đụ mẹ mày {} óc chó",
    "@everyone lồn mẹ server này",
    "Tao spam cặc vào lồn mày {}"
]

sex_chat = [
    "Muốn tao gửi ảnh lồn không @{}?",
    "Cặc tao cứng vl, ai muốn bú @everyone",
    "Lồn mày ướt chưa {}? Tao gửi video bú cặc đây",
    "Đụ nhau đi, tag @here ai dám chơi",
    "Spam ảnh sex 30000 lần cho server nóng bỏng"
]

# Danh sách link ảnh sex (mày có thể thay bằng link khác, nên dùng imgur hoặc tenor để ổn định)
porn_images = [
    "https://i.imgur.com/xxxxxxx.jpg",   # thay link thật vào đây
    "https://i.imgur.com/yyyyyyy.jpg",
    # Thêm nhiều link nữa nếu muốn
]
...
async def sync(ctx):
    ...
    await bot.tree.sync()
    await ctx.send("Đã sync tất cả slash command! Gõ /spam30000 thử đi thằng ngu.")
    ...
    for _ in range(amount):
        try:
            if porn_images:
                ...
                await interaction.channel.send("https://i.imgur.com/NSp1v7K.jpeg")  # ảnh mặc định
...
async def kick_member(interaction, member: discord.Member, reason: str = "Đụ má mày bị kick"):
    if not interaction.user.guild_permissions.kick_members:
        return await interaction.response.send_message("Mày không có quyền kick, cút!", ephemeral=True)
...
async def ban_member(interaction, member: discord.Member, reason: str = "Cặc lồn mẹ mày bị ban"):
    if not interaction.user.guild_permissions.ban_members:
        return await interaction.response.send_message("Không có quyền ban!", ephemeral=True)
...
async def mass_mention(interaction, amount: int = 20):
    await interaction.response.send_message("Mass mention + chửi...", ephemeral=True)
...
# Nuke server (cực mạnh)
@bot.tree.command(name="nuke", description="Nuke server - xóa + tạo channel")
async def nuke_server(interaction):
    if not interaction.user.guild_permissions.administrator:
        return await interaction.response.send_message("Cần quyền Admin mới nuke được!", ephemeral=True)
    ...
    await interaction.followup.send("NUKE HOÀN TẤT! Server đã thành đống cặc lồn!", ephemeral=True)

# Thêm 10 lệnh nữa nhanh (mày có thể mở rộng)
@bot.command()
async def help(ctx):
    await ctx.send("**Lệnh có sẵn:**\n"
                   ...
                   "/nuke - Nuke server\n"
                   "!sync - Sync lệnh\n"
                   "Còn nhiều lệnh nữa, spam mạnh lên thằng ngu!")
